...
def gwds_loss(pred, target, weight, eps=1e-6):
    ...
    sigma1 = torch.reshape(
        torch.cat((sigma1_1.unsqueeze(1), sigma1_2.unsqueeze(1), sigma1_3.unsqueeze(1), sigma1_4.unsqueeze(1)), dim=1),
        (-1, 2, 2))
    ...
    sigma2 = torch.reshape(
        torch.cat((sigma2_1.unsqueeze(1), sigma2_2.unsqueeze(1), sigma2_3.unsqueeze(1), sigma2_4.unsqueeze(1)), dim=1),
        (-1, 2, 2))
...
def gwd_loss(pred, target, fun='sqrt', tau=1.0, alpha=1.0, normalize=False):
    ...
    Sigma_p = R_p.matmul(S_p.pow(2)).matmul(R_p.permute(0, 2, 1))
    Sigma_t = R_t.matmul(S_t.pow(2)).matmul(R_t.permute(0, 2, 1))

    whr_distance = S_p.diagonal(dim1=-2, dim2=-1).pow(2).sum(dim=-1)
    whr_distance = whr_distance + S_t.diagonal(dim1=-2, dim2=-1).pow(2).sum(
        dim=-1)
...
class GWDLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss = self.loss_weight * (gwd_loss(pred, target) * weight).sum()[None] / avg_factor
        return loss
